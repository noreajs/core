  ...
  return {
    message: (_value, field, _origin, def, data) => {
      ...
      if (Array.isArray(dependencies)) {
        return `\`${field}\` field value must be before the following fields: ${dependencies
          .map((d) => `\`${d}\``)
          .join(", ")}`;
      } else {
        return `\`${field}\` field value must be before \`${dependencies}\``;
      }
    },
      ...
      const singleTest = (dependencyValue: any) => {
        ...
        if (isFilled(dependencyValue)) {
          switch (type) {
            ...
            case "date":
              if (strict) {
                return moment(value).isBefore(moment(dependencyValue));
              } else {
                return moment(value).isSameOrBefore(moment(dependencyValue));
              }
...
export default beforeFieldsRule;
